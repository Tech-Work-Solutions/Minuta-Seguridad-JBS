  <template>
    ...
    <div class="p-6 bg-gray-100">
        ...
        <div class="flex flex-wrap items-center">
            ...
            <em class="fas fa-calendar-check"></em> Gestión de Permisos
            </h1>
        </div>
        <form v-if="!esGuarda">
        ...
        <form @submit.prevent="handleSubmit" class="bg-white rounded-lg shadow p-6">
                ...
                <v-row class="fill-height">
                    ...
                    <v-sheet height="64">
                        ...
                        <div class="flex overflow-x-auto space-x-2">
                            <v-btn outlined class="mr-4" color="grey darken-2" @click="openDialog" v-if="esGuarda || gestionarPermiso">
                            Solicitar Permiso
                            </v-btn>
                    ...
                    <v-sheet height="800">
                        ...
                        <v-dialog v-model="dialog" max-width="500px" @click:outside="resetForm">
                            <v-card>
                                <v-card-title>
                                <span class="headline">{{ isEditing ? 'Editar permiso' : 'Solicitud de permisos' }}</span>
                                </v-card-title>
                                <v-card-text>
                                <v-text-field
                                ...
                                <v-select
                                    ...
                                    :error-messages="!$v.newEvent.sede_id.$pending && !$v.newEvent.sede_id.$model && $v.newEvent.sede_id.$dirty ? ['Seleccione la sede'] : []"                    
                                ></v-select>
                                ...
                                <v-select
                                    ...
                                    item-value="id"
                                    item-text="name"
                                    v-if="gestionarPermiso"
                                    label="Seleccionar estado del permiso"
                                    :error-messages="!$v.newEvent.estado.$pending && !$v.newEvent.estado.$model && $v.newEvent.estado.$dirty ? ['Seleccione el estado del permiso'] : []"                    
                                ></v-select>
                                </v-card-text>
            ...
            <div class="flex mb-2 mt-1" v-if="esGuarda || gestionarPermiso">
                <button
    ...
    export default {
      ...
      methods: {
        submitForm() {
            ...
            let hasErrors = this.$v.newEvent.name.$invalid ||
            ...
            if (!this.esGuarda) {
                hasErrors = hasErrors ||
                            this.$v.newEvent.user_id.$invalid ||
                            this.$v.newEvent.sede_id.$invalid ||
                            this.$v.newEvent.estado.$invalid;
            }
        ...
        async handleSubmit() {
          try {
            ...
            const eventosAInsertar = this.newEvents.map(evento => ({
              ...
              sede_id: this.esGuarda ? this.sedeId : evento.sede_id,
