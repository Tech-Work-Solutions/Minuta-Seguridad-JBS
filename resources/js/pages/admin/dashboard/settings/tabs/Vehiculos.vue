        ...
        <form>
          <div class="flex flex-wrap mt-5">
            <div class="w-full">
                ...
                <p class="text-red-500 text-sm" v-if="submited && !$v.formData.placa.required">Ingrese la placa del vehículo</p>
                </div>
            </div>          
          </div>
          <div class="flex mb-4">
      ...
      <div class="flex-auto p-3 lg:p-5">
        ...
        <div class="block w-full overflow-x-auto mb-12">
          ...
          <table class="items-center w-full bg-gray-100 border-collapse">
            ...
            <tbody>
              <tr class="bg-gray-100 text-left" v-for="(vehiculo, index) in searchVehiculo" :key="vehiculo.id">
                <td
...
export default {
  ...
  methods: {
    ...
    async register(){
      await axios.post('/api/registerVehiculo', this.formData).then((response) => { 
        this.spiner = false;
        this.submited = false;
        this.formData.placa = '';
    ...
    getVehiculos(){
      axios.get('/api/getVehiculos').then((response) => {
  ...
  computed: {
    searchVehiculo() {
      return this.vehiculos.filter((ve) => (ve.placa).toUpperCase().includes(this.search.toUpperCase()));
    }
  }
}
</script>