        ...
        <form>
          <div class="flex flex-wrap mt-5">
            <div class="w-full">
                ...
                <p class="text-red-500 text-sm" v-if="submited && !$v.formData.nombre.required">Ingrese el nombre de la EPS</p>
                </div>
            </div>          
          </div>
          <div class="flex mb-4">
      ...
      <div class="flex-auto p-3 lg:p-5">
        ...
        <div class="block w-full overflow-x-auto mb-12">
          ...
          <table class="items-center w-full bg-gray-100 border-collapse">
            ...
            <tbody>
              <tr class="bg-gray-100 text-left" v-for="(item, index) in searchEps" :key="item.id">
                <td
...
export default {
  ...
  methods: {
    ...
    async register(){
      await axios.post('/api/registerEps', this.formData).then((response) => { 
        this.spiner = false;
        this.submited = false;
    ...
    getEps(){
      axios.get('/api/getEps').then((response) => {
  ...
  computed: {
    searchEps() {
      return this.eps.filter((e) => (e.nombre).toUpperCase().includes(this.search.toUpperCase()));
    }
  }
}
</script>